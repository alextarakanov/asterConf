...
/**
 * GenStatisticsBSearch represents the model behind the search form of `app\models\GenStatisticsB`.
 */
class GenStatisticsBSearch extends GenStatisticsB
{
    ...
    {
        return [
            [['idStatistics', 'projectId', 'bNumber', 'sim_name', 'usedSecondPerDay', 'usedMinutePerDay', 'usedCountAnsweredCallPerDay', 'usedCountRepeatErrorPerDay'], 'integer'],
    ...
    {
        $query = GenStatisticsB::find();
//        ->select([bNumber,sim_name,sum(usedMinutePerDay),floor(sum(usedSecondPerDay)/60), sum(usedCountAnsweredCallPerDay),sum(usedCountRepeatErrorPerDay)])
//        ->select(['projectId','bNumber','sim_name','usedMinutePerDay','usedSecondPerDay','usedCountAnsweredCallPerDay','usedCountRepeatErrorPerDay'])
//        ->groupBy(['bNumber'])
//        ->all();
...
//        var_dump($params);die;
        ...
        $query = GenStatisticsB::find()
            ->select(['projectId', 'bNumber', 'sim_name', 'sum(usedMinutePerDay) as usedMinutePerDay', 'floor(sum(usedSecondPerDay)/60) as usedSecondPerDay', 'sum(usedCountAnsweredCallPerDay) as usedCountAnsweredCallPerDay', 'sum(usedCountRepeatErrorPerDay) as usedCountRepeatErrorPerDay'])
...
//        var_dump($this->dayStart);die;
        ...
        $query = GenStatisticsB::find()
            ->select(['projectId', 'bNumber', 'sim_name', 'usedMinutePerDay', 'floor(usedSecondPerDay/60) as usedSecondPerDay', 'usedCountAnsweredCallPerDay', 'usedCountRepeatErrorPerDay', 'createDatetime'])
            ->where(['between', 'createDatetime', "$this->yearStart-$this->monthStart-$this->dayStart 00:15:00", "$this->yearEnd-$this->monthEnd-$this->dayEnd  00:15:00"])
            ->andWhere(['bNumber' => $this->bNumberCheck]);
        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);
        $this->load($params);
